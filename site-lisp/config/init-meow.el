...
(setq +evan/meow-motion-mode-alist nil) ; 为motion特殊处理过的mode列表

(defun +evan/meow-add-motion-mode-alist (mode)
  (unless (-contains-p +evan/meow-motion-mode-alist mode)
    (push mode +evan/meow-motion-mode-alist)))
...
(add-hook 'meow-motion-mode-hook (lambda ()
				   ...
				   (unless (-contains-p +evan/meow-motion-mode-alist major-mode)
				     (meow-setup))))
